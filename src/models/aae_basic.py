...
dataset = ImageDataset(
    directory='data/preprocessed_images_64_with_gray_scale', transform=transform)
...
for epoch in range(args.n_epochs):
    ...
    sample_image(n_row=3, epoch=epoch,
                 img_dir=f"{args.img_dir}_{args.img_size}")

now = datetime.now().strftime("%Y-%m-%d_%H:%M")
df = pd.DataFrame(train_logs)
df.to_csv(f"logs/aae_basic_train_logs_{now}.csv")
torch.save(encoder, f'trained_models/aae_basic_encoder_{now}.pth')
